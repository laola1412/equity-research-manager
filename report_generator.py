...
def create_report(save_name, name, close, date, target_value, rating, description=description_qrtea, operations_txt=operations_example):
    styles = getSampleStyleSheet()
    styles.add(ParagraphStyle(name="TimesNewRoman", fontName="TimesNewRoman", bold=None, italic=None))
    ...
    report_title = Paragraph(f"{name.title()} Report", styles["h1"])
    ...
    description = [Paragraph("About", styles["h2"]), Paragraph(description, styles["BodyText"])]
    operations = [Paragraph("Operations", styles["h2"]), Paragraph(operations_txt, styles["BodyText"])]
    empty_line = Spacer(1,20)
    
    doc.build([report_title, 
                  report_info, 
                  report_rating, 
                  empty_line, 
